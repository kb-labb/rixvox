...
from utils.strings_hr import abbreviations, number_to_text, sym_ignore, symbols
...
def check_time(words: List[Word]) -> bool:
    if (
        words[0].type == WordType.number
        and words[2].type == WordType.number
        and len(words[0].text) <= 2
        and len(words[2].text) == 2
        and words[1].text in [":", ","]
    ):
        return True
    else:
...
def join(words: List[Word]) -> List[Word]:
    ...
    while words:
        if len(words) >= 3 and check_time(words[:3]):
            ...
            len(words) > 1
            and words[0].type == WordType.number
            and words[1].type == WordType.number
        ):
            sw = []
